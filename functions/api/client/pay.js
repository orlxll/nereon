...
async function ensureCommercialRecords(env,p){if(!p||p.status!=='accepted')return;const existing=await env.DB.prepare('SELECT id FROM contracts WHERE proposal_id=? LIMIT 1').bind(p.id).first();if(existing)return;const cid='ctr_'+crypto.randomUUID(),iid='inv_'+crypto.randomUUID(),now=new Date().toISOString(),due=new Date(Date.now()+14*24*3600*1000).toISOString(),environment=String(p.environment||'test');try{await env.DB.batch([env.DB.prepare(`INSERT INTO contracts(id,proposal_id,lead_id,status,title,amount_eur,currency,accepted_at,created_at,updated_at,environment) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(cid,p.id,p.lead_id,'accepted',p.title,p.price_eur,p.currency,now,now,now,environment),env.DB.prepare(`INSERT INTO invoices(id,contract_id,lead_id,amount_eur,currency,status,due_date,created_at,updated_at,environment) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(iid,cid,p.lead_id,p.price_eur,p.currency,'pending',due,now,now,environment)]);}catch(e){const check=await env.DB.prepare('SELECT id FROM contracts WHERE proposal_id=? LIMIT 1').bind(p.id).first();if(!check)throw e;}}
...
export async function onRequestPost({ request, env }) {
  ...
  if (String(proposal.environment || 'test') !== environment) return json({ ok: false, error: 'payment_environment_mismatch' }, 409);
    ...
    const payment = {
      ...
      provider: 'stripe',
      lead_id: String(proposal.lead_id),
    ...
    await env.DB.prepare(`INSERT INTO payments
      (id,invoice_id,lead_id,provider,provider_payment_id,checkout_url,amount_eur,currency,status,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)`).bind(payment.id,payment.invoice_id,payment.lead_id,payment.provider,payment.provider_payment_id,payment.checkout_url,payment.amount_eur,payment.currency,payment.status,payment.created_at,payment.updated_at).run();
